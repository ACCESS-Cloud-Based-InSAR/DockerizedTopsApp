...
def add_2d_layer(
    layer_name: str, gunw_netcdf_path: Path, additional_attrs: Optional[dict] = None
) -> Path:
    """
    ...
    if not isinstance(band_number, int) or (band_number < 1):
        ValueError("Layers must select individual layers from outputs i.e 1, 2, ...")
    ...
    ds = xr.open_dataset(layer_data["input_relative_path"], engine="rasterio")
    ...
    ds["latitude"].attrs.update({"long_name": "latitude", "standard_name": "latitude"})
    ...
    ds.to_netcdf(gunw_netcdf_path, group=layer_data["dst_group"], mode="a")
