...
def get_geocoded_coords_ISCE2(args):
    ...
    Note this function is specific for ISCE 2.0 where there is an inconsistency for the
    pixel definition in the vrt's. Isce assumes center pixel while the coordinate and transf
    reconstruction required the input to be edge defined."""
...
def get_h5_dataset_coords(args):
    ...
    for geovariable in geovariables:
        ...
        if (varname == 'longitude' or varname == 'Longitude' or
            varname == 'lon' or varname == 'Lon' or varname == 'lons' or
            varname == 'Lons'):
            lons = get_h5_dataset([h5_file, variable])
            count += 1
            lons_map = geovariable[1]
        elif (varname == 'latitude' or varname == 'Latitude' or
              varname == 'lat' or varname == 'Lat' or varname == 'lats' or
              varname == 'Lats'):
            lats = get_h5_dataset([h5_file, variable])
            count += 1
            lats_map = geovariable[1]
        elif (varname == 'height' or varname == 'Height' or
              varname == 'h' or varname == 'H' or varname == 'heights'
              or varname == 'Heights'):
