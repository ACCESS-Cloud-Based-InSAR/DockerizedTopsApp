        ...
        if isinstance(data, str):
            ...
            if len(properties_data.dims) == 1:
                dset = fid.createVariable(
                    properties_data.name, properties_data.type, (properties_data.dims[0]), fill_value=nodata, zlib=True
                )
...
def create_dataset(fid, dataset, fid_parent=None):
    ...
    # loading data from a src file
        ...
        data, data_transf, data_proj, data_nodata = data_loading(
            properties_data.src_file, properties_data.type, properties_data.band
        )
    ...
    if (
        properties_data.name.lower() == "connected_components"
        or properties_data.name.lower() == "connectedcomponents"
        or properties_data.name.lower() == "coherence"
    ):
        # setting the no-data value in case the user is not overwriting it
...
def CF_attribute_compliance(properties_data, name):
    ...
    if len(CF_missing_attr_name) > 0:
        if class_flag:
            properties_data.attribute = expand_attrdict(
                properties_data.attribute, CF_missing_attr_name, CF_missing_attr_value
            )
        else:
...
def createParser():
    ...
    parser.add_argument(
        "-i", "--input", dest="filename", type=str, required=True, help="Input json file to be used for packing"
    )
    return parser
