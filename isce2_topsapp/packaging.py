...
def product_naming_scheme(product: str) -> tuple[str, str, str]:
    if product == 'GUNW':
...
def read_baselines(tops_proc_xml: str) -> dict:
    with Path(tops_proc_xml).open() as f:
        xml_str = f.read()
...
def update_gunw_internal_version_attribute(nc_path: Path, new_version: str = '1c') -> None:
...
def get_gunw_hash_id(reference_ids: list, secondary_ids: list) -> str:
    all_ids = json.dumps([' '.join(sorted(reference_ids)), ' '.join(sorted(secondary_ids))]).encode('utf8')
...
def make_geocube(isce_data_directory: str | Path) -> Path:
    """Generate the `metadata.h5` from a command-line script.
...
def _write_json_config(*, product: str, gunw_id: str, directory: Path) -> Path:
    """Read the json template and write a new entry: `file: '<gunw_id>.nc'`.
...
def perform_netcdf_packaging(*, product: str, gunw_id: str, isce_data_dir: str | Path) -> Path:
...
def package_additional_layers_into_gunw(
    ...
    additional_attributes_lst = [additional_attributes.get(layer_name, None) for layer_name in additional_2d_layers]
    zipped_data = zip(additional_2d_layers, additional_attributes_lst)
    [add_2d_layer(layer, gunw_path, additional_attrs=add_attrs) for (layer, add_attrs) in zipped_data]
...
def get_geocoded_layer_means(*, merged_dir: Path | str | None = None) -> dict:
    ...
    mean_vals = {
        'mean_filtered_coherence_without_water_mask': get_layer_mean_p('filtered_coherence', apply_water_mask=False),
        'mean_filtered_coherence_with_water_mask': get_layer_mean_p('filtered_coherence', apply_water_mask=True),
        'mean_unfiltered_coherence_without_water_mask': get_layer_mean_p(
            'unfiltered_coherence', apply_water_mask=False
        ),
        'mean_unfiltered_coherence_with_water_mask': get_layer_mean_p('unfiltered_coherence', apply_water_mask=True),
...
def record_params_as_global_attrs(*, netcdf_path: Path, cmd_line_str: str, topsapp_params: dict) -> Path:
...
def record_stats_as_global_attrs(*, netcdf_path: Path, isce_data_dir: Path | str) -> Path:
    """Record summary statistics of the ISCE2 outputs as global netcdf attributes.
...
def package_gunw_product(
    *,
    ...
    additional_2d_layers: list | None = None,
    standard_product: bool = True,
    additional_attributes: dict | None = None,
) -> Path:
    """Create a GUNW standard product or COSEIS-SAR product netcdf from the ISCE outputs and some initial metadata.
    ...
    out_nc_file = perform_netcdf_packaging(product=product, isce_data_dir=isce_data_directory, gunw_id=gunw_id)
