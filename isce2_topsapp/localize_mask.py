...
def download_water_mask(
    extent: list,
    water_mask_name: str = "pekel_water_occurrence_2021",
    buffer: float = 0.1,
) -> dict:
    ...
    if water_mask_name == "esa_world_cover_2021_10m":
        X, p = get_raster_from_tiles(
            extent_buffered, tile_shortname="esa_world_cover_2021"
        )
        ...
        X, p = get_raster_from_tiles(
            extent_buffered, tile_shortname="pekel_water_occ_2021"
        )
        ...
        if (np.abs(lats) < 59.9).all():
            ...
            print("Request out of SWBD coverage ", "Skip downloading water mask!!")
            mask_filename = ""
