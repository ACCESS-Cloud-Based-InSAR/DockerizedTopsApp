...
def tag_dem_xml_as_ellipsoidal(dem_path: Path) -> str:
    ...
    root.insert(0, y)
    with Path(xml_path).open('wb') as file:
        file.write(etree.tostring(root, pretty_print=True))
    return xml_path


def fix_image_xml(isce_raster_path: str) -> str:
    fix_cmd = ['fixImageXml.py', '-i', str(isce_raster_path), '--full']
...
def download_dem_for_isce2(
    ...
    full_res_dem_dir: Path | None = None,
    low_res_dem_dir: Path | None = None,
    buffer: float = 0.4,
) -> dict:
    """Download and prepare full and low resolution DEMs for ISCE2.
    ...
    dem_array, dem_profile = stitch_dem(
        ...
        # 'Point' + 'aria-legacy' reproduce the pre-3.0.0 dem-stitcher DEM for time-series
        # consistency with the existing ARIA GUNW archive
    ...
    if geocode_resolution == 90:
        ...
        dem_geocode_arr, dem_geocode_profile = reproject_arr_to_match_profile(
            dem_array,
            dem_profile_isce,
            dst_profile,
            num_threads=5,
    ...
    return {
        'extent_buffered': extent_buffered,
