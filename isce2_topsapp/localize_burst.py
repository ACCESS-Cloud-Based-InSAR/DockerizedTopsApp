...
class BurstMetadata:
    def __init__(
        self, metadata: ET.Element, manifest: ET.Element, burst_params: BurstParams
    ):
        ...
        image_numbers = [
            int(x.attrib["source_filename"].split("-")[-1][2]) for x in metadata
        ]
        ...
        for name in files:
            ...
            content.tag = "product"  # type: ignore[union-attr]
        ...
        self.measurement_name = [
            Path(x).name for x in file_paths if re.search(pattern, x)
        ][0]

        self.gcp_df = self.create_gcp_df()
        self.footprint = self.create_geometry(self.gcp_df)[0]
        self.swath = int(self.annotation.findtext(".//{*}adsHeader/swath")[2])  # type: ignore[attr-defined]
        self.polarisation = self.annotation.findtext(".//{*}adsHeader/polarisation")  # type: ignore[attr-defined]
        self.orbit_direction = self.manifest.findtext(".//{*}pass").lower()  # type: ignore[union-attr]
    ...
    def create_geometry(self, gcp_df):
        ...
        first_line = gcp_df.loc[
            gcp_df["line"] == burst_index * lines, ["longitude", "latitude"]
        ]
        second_line = gcp_df.loc[
            gcp_df["line"] == (burst_index + 1) * lines, ["longitude", "latitude"]
        ]
...
def download_geotiff(
    asf_session: requests.Session, burst_params: BurstParams, out_file: Union[Path, str]
) -> str:
    burst_request = create_burst_request(burst_params, content="metadata")
    burst_request["cookies"] = {"asf-urs": asf_session.cookies["asf-urs"]}
...
def download_manifest(
    safe_url: str, out_file: Optional[Union[Path, str]] = None
) -> ET.Element:

    safe_name = Path(safe_url).with_suffix(".SAFE").name

    my_netrc = netrc.netrc()
    username, _, password = my_netrc.authenticators("urs.earthdata.nasa.gov")  # type: ignore[misc]
...
def spoof_safe(
    ...
    ET.ElementTree(burst.annotation).write(  # type: ignore
        annotation_path / burst.annotation_name, **et_args  # type: ignore
    )
    ET.ElementTree(burst.calibration).write(  # type: ignore
        calibration_path / burst.calibration_name, **et_args  # type: ignore
    )
    ET.ElementTree(burst.noise).write(calibration_path / burst.noise_name, **et_args)  # type: ignore
    ET.ElementTree(burst.manifest).write(safe_path / "manifest.safe", **et_args)  # type: ignore

    if download_strategy == "single_burst":
        burst_params = BurstParams(
            safe_url=burst.safe_url,
            image_number=burst.image_number,
            burst_number=burst.burst_number,
        )
        download_geotiff(
            asf_session, burst_params, measurement_path / burst.measurement_name
        )
    elif download_strategy == "surrounding_burst":
        n_bursts = len(burst.annotation.find(".//burstList"))  # type: ignore
        names = {
        ...
        raise NotImplementedError(
            f"Download strategy {download_strategy} is not implemented, check spelling."
        )
...
def get_asf_session() -> requests.Session:
    ...
    response = session.get(
        "https://urs.earthdata.nasa.gov/oauth/authorize", params=payload
    )
    response.raise_for_status()
    return session
