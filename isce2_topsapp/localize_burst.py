...
class BurstMetadata:
    def __init__(self, metadata: ET.Element, manifest: ET.Element, burst_params: BurstParams) -> None:
    ...
    @staticmethod
    def reformat_gcp(point: ET.Element) -> dict[str, float]:
    ...
    def create_geometry(
        self, gcp_df: pd.DataFrame
    ) -> tuple[geometry.Polygon, tuple[float, float, float, float], tuple[float, float]]:
...
def download_geotiff(asf_session: requests.Session, burst_params: BurstParams, out_file: Path | str) -> str:
    burst_request = create_burst_request(burst_params, content='metadata')
    burst_request['cookies'] = {'asf-urs': asf_session.cookies['asf-urs']}
...
def download_manifest(safe_url: str, out_file: Path | str = None) -> ET.Element:
...
def spoof_safe(
    ...
    """Create a spoofed SAFE directory for a burst.
...
def download_bursts(param_list: Iterator[BurstParams]) -> list[BurstMetadata]:
    """Download the bursts described by the given parameters.
