...
def get_state_vector_arrays(orbit_xml: Union[str, Path]) -> list[StateVector]:
    ...
    for idx in range(len(t)):
        ...
        state_vector = StateVector(time=t[idx], position=position, velocity=velocity)
        state_vector_list.append(state_vector)

    return state_vector_list


def get_orbit_obj_from_orbit_xmls(
...
def compute_solid_earth_tide_from_gunw(
    *, gunw_path: str, reference_or_secondary: str, orbit_xmls: list[Path]
    ...
    with xr.open_dataset(gunw_path, group=group, mode="r") as ds:
        ...
        latitude_coord_arr = ds.latitudeMeta.data + lat_res / 2.0
        longitude_coord_arr = ds.longitudeMeta.data - lon_res / 2.0
        ...
        tide_e, tide_n, tide_u = compute_enu_solid_earth_tide(
...
def solid_grid_pixel_rounded_to_nearest_sec(
    ...
    lat_step = -res_y
    lon_step = res_x
    tide_e, tide_n, tide_u = solid_grid(
        timestamp.year,
        timestamp.month,
        timestamp.day,
        timestamp.hour,
...
def solid_grid_pixel_interpolated_across_second_est(
...
def compute_enu_solid_earth_tide(
    *,
    ...
    tide_e_flat, tide_n_flat, tide_u_flat = (
        np.array(tide_e_flat),
...
def compute_los_solid_earth_tide(
    tide_e: np.array,
    tide_n: np.array,
    tide_u: np.array,
    inc_angle: np.array,
...
def update_gunw_with_solid_earth_tide(
    ...
    solid_earth_tide_ds = compute_solid_earth_tide_from_gunw(
        gunw_path=gunw_path,
        orbit_xmls=orbit_xmls,
