...
def test_gunw_id_generation_crossing_dateline(product: str = 'GUNW') -> None:
    sample_json_path = test_dir / 'midnight_crossing_metadata.json'
    metadata = json.load(sample_json_path.open())
    gunw_id = get_gunw_id(
...
def test_extract_baselines(tops_proc_xml_path: Path) -> None:
...
def test_mean_of_geocoded_isce_outputs() -> None:
    """Use the data in the public bucket to verify that the layer means are correct."""
    ...
    assert np.abs(out['mean_filtered_coherence_without_water_mask'] - gunw_mean_data['coherence']) < 0.01
    assert np.abs(out['mean_unfiltered_coherence_without_water_mask'] - gunw_mean_data['unfilteredCoherence']) < 0.01
