...
def test_mean_of_geocoded_isce_outputs():
    ...
    vars_data_0 = {
        var: f"//science/grids/data/{var}"
        for var in ["unfilteredCoherence", "coherence"]
    }
    vars_data_1 = {
        var: f"//science/grids/imagingGeometry/{var}"
        for var in ["azimuthAngle", "incidenceAngle"]
    }
    ...
    assert (
        np.abs(
            out["mean_filtered_coherence_without_water_mask"]
            - gunw_mean_data["coherence"]
        )
        < 0.01
    )
    assert (
        np.abs(
            out["mean_unfiltered_coherence_without_water_mask"]
            - gunw_mean_data["unfilteredCoherence"]
        )
        < 0.01
    )
    ...
    assert (
        out["mean_filtered_coherence_without_water_mask"]
        < out["mean_filtered_coherence_with_water_mask"]
    )
    assert (
        out["mean_unfiltered_coherence_without_water_mask"]
        < out["mean_unfiltered_coherence_with_water_mask"]
    )
