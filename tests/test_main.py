...
def test_main_check_earthdata_credentials_prefer_netrc(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_main_check_earthdata_credentials_fn_variables(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv('HOME', str(tmp_path))
    netrc = tmp_path / '.netrc'
...
def test_main_check_earthdata_credentials_env_variables(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv('HOME', str(tmp_path))
    netrc = tmp_path / '.netrc'
...
def test_true_false_string_argument() -> None:
...
def test_esd_threshold_argument() -> None:
